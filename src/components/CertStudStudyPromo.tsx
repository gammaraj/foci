...
export default function CertStudStudyPromo({
    ...
    <>
      ...
      <p
        className={`mt-1 font-semibold text-slate-800 dark:text-slate-100 ${
          variant === "inline" ? "text-base" : "text-sm sm:text-base"
        }`}
      >
  ...
  return (
    <aside
      className={`relative mx-3 mb-3 rounded-xl border border-emerald-200/80 dark:border-emerald-900/50 bg-emerald-50/50 dark:bg-emerald-950/20 px-3 py-3 ${className}`}
      aria-label="Partner link: CertStud certification practice"
    >
      <button
        type="button"
