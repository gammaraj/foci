...
interface TaskEditButtonProps {
  ...
  taskTitle?: string;
  className?: string;
  /** Icon-only on small screens to save row space in card view. */
  compact?: boolean;
}
...
export function TaskEditButton({ onClick, isOpen, taskTitle, className = "", compact = false }: TaskEditButtonProps) {
    ...
    return (
      <>
        <button
          type="button"
          onClick={onClick}
          className={`sm:hidden shrink-0 p-1 rounded-md border transition-colors ${editBtnClass(!!isOpen)} ${className}`}
        ...
        <button
          type="button"
          onClick={onClick}
          className={`hidden sm:inline-flex shrink-0 px-1.5 py-0.5 text-xs font-semibold rounded-md border transition-colors whitespace-nowrap ${editBtnClass(!!isOpen)} ${className}`}
          aria-expanded={isOpen}
          aria-label={isOpen ? `Close task details${named}` : `Edit task${named}`}
          title={isOpen ? "Close details" : "Edit task"}
        >
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`shrink-0 px-1.5 py-0.5 text-xs font-semibold rounded-md border transition-colors whitespace-nowrap ${editBtnClass(!!isOpen)} ${className}`}
      aria-expanded={isOpen}
      aria-label={isOpen ? `Close task details${named}` : `Edit task${named}`}
      title={isOpen ? "Close details" : "Edit task"}
    >
