...
export function TaskExpansionDrawer({
  ...
  return (
    <>
      <button
        type="button"
        className="fixed inset-0 z-40 bg-slate-900/40 dark:bg-black/60 cursor-default"
      ...
      <aside
        ...
        className="fixed z-50 inset-x-0 bottom-0 w-full max-w-full min-w-0 box-border sm:inset-x-auto sm:left-auto sm:right-3 sm:top-3 sm:bottom-3 sm:w-[min(calc(100%-1.5rem),40rem)] flex flex-col max-h-[min(94dvh,100%)] sm:max-h-none rounded-t-2xl sm:rounded-2xl border surface-panel bg-surface-elevated shadow-2xl shadow-slate-900/15 overflow-hidden isolate"
        style={{
          paddingLeft: "env(safe-area-inset-left, 0px)",
          paddingRight: "env(safe-area-inset-right, 0px)",
        }}
      >
        <header className="flex items-start gap-3 px-4 sm:px-6 py-3.5 sm:py-4 border-b border-slate-200/80 dark:border-surface-border shrink-0 min-w-0">
          <div className="min-w-0 flex-1 overflow-hidden">
            <p className="app-section-label text-slate-500 dark:text-slate-400 mb-1">
              Task details
            </p>
            {canEditTitle ? (
              <div className="group/title flex items-start gap-1.5 min-w-0">
                <textarea
                  ...
                  className={`flex-1 min-w-0 text-lg sm:text-xl font-semibold leading-snug outline-none transition-colors break-words [overflow-wrap:anywhere] resize-none overflow-hidden ${
                    isEditingTitle
                      ? "rounded-md border border-blue-400 bg-surface-elevated dark:bg-surface-recessed text-slate-900 dark:text-white px-2 py-1 focus-visible:ring-2 focus-visible:ring-blue-400/40"
