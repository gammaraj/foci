      ...
      <div className={className}>
        {templates.map((tpl) => (
          <button
            key={tpl.label}
            type="button"
            className="w-full text-left px-3 py-2 text-slate-700 dark:text-slate-200 hover:bg-surface-muted dark:hover:bg-surface-hover transition-colors border-b border-slate-50 dark:border-surface-border/50 last:border-b-0"
          ...
          <div className="grid grid-cols-2 gap-2">
            {items.map((tpl) => (
              <button
                ...
                type="button"
                onClick={() => onSelect(tpl)}
                className="text-left p-3 rounded-xl border border-slate-100 dark:border-surface-border hover:border-blue-300 dark:hover:border-blue-600 hover:bg-blue-50/50 dark:hover:bg-blue-900/20 transition-all group"
              >
  ...
  return (
    <div className={`space-y-1.5 min-w-0 ${className}`}>
      <div className="flex items-baseline justify-between gap-2 px-0.5">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 shrink-0 roomy:hidden">Swipe for more</p>
      </div>
      <div
