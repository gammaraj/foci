  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        className="relative nav-chrome-icon-btn"
        aria-label={
          badgeCount > 0
      ...
      {showPanel && (
        <div
          ...
          className="absolute right-0 top-full mt-1.5 z-[80] w-[min(20rem,calc(100vw-1.5rem))] max-h-[min(24rem,70vh)] overflow-y-auto rounded-xl border border-surface-border bg-surface-elevated shadow-xl"
        >
          <div className="sticky top-0 flex items-center justify-between gap-2 px-3 py-2.5 border-b border-slate-200/80 dark:border-surface-border bg-surface-elevated/95 dark:bg-surface-elevated/95 backdrop-blur-sm">
            <div>
              ...
              {items.map(({ task, projectName, kind, daysLate }) => (
                <li key={task.id}>
                  <button
                    ...
                    className="w-full text-left px-3 py-2.5 hover:bg-slate-50 dark:hover:bg-surface-hover transition-colors"
                  >
