...
export function doneMascotCaption(todayCount: number, idleDays: number | null): string {
  if (todayCount > 0) {
    return todayCount === 1
      ? "Beavy is buzzing — 1 task done today"
      : `Beavy is buzzing — ${todayCount} tasks done today`;
  }
  if (idleDays == null) return "Beavy is ready when you finish a task";
  if (idleDays <= 1) return "Beavy is waiting for today’s first finish";
  if (idleDays === 2) return "Beavy misses a checkmark — 2 days quiet";
  return `Beavy’s looking wilted — ${idleDays} days without a finish`;
}
