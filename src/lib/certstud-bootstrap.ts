...
import {
  buildCertStudProjectName,
  buildCertStudTaskTitle,
  parseDurationMinutes,
  toCertStudContext,
  type CertStudContext,
  type CertStudDeepLinkParams,
...
export interface CertStudBootstrapResult {
  taskId: string | null;
  durationMinutes: number | null;
  context: CertStudContext;
}

export async function bootstrapCertStudSession(
  params: CertStudDeepLinkParams
): Promise<CertStudBootstrapResult> {
  const context = toCertStudContext(params);
  const durationMinutes = parseDurationMinutes(params.duration);
  let taskId: string | null = null;

  if (params.cert || params.certCode || params.topic) {
    const projects = await loadProjects();
    const projectName = buildCertStudProjectName(params);
    ...
    if (!project) {
      ...
      project = {
        ...
        description: params.returnUrl
          ? `CertStud practice sessions — ${params.returnUrl}`
          : "CertStud certification study",
        createdAt: Date.now(),
      } satisfies Project;
    ...
    const taskTitle = buildCertStudTaskTitle(params);
    let task = tasks.find(
    ...
    if (!task) {
      ...
      task = {
        ...
        order: maxOrder + 1,
        description: params.returnUrl
          ? `Continue on CertStud → ${params.returnUrl}`
          : undefined,
      } satisfies Task;
