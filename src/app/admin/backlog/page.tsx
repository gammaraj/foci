...
function Pill({ className, children }: { className: string; children: ReactNode }) {
  return (
    <span className={`inline-flex items-center rounded-md px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide ${className}`}>
      {children}
    </span>
  );
}
...
function ItemCard({ item }: { item: BacklogItem }) {
  return (
    <article className="rounded-xl border border-slate-200/90 dark:border-surface-border bg-surface-elevated/80 dark:bg-surface-elevated/80 px-4 py-3">
    ...
    <main className={`${ADMIN_SHELL} py-8 space-y-10`}>
      ...
      <dl className="grid grid-cols-1 min-[400px]:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
        {[
          ...
          <div
            key={stat.label}
            className="rounded-xl border border-slate-200/90 dark:border-surface-border bg-surface-elevated/80 dark:bg-surface-elevated/80 px-3 py-3"
          >
      ...
      <section aria-labelledby="goals-heading" className="space-y-3">
        ...
        <ol className="space-y-2">
          {PRODUCT_GOALS.map((goal) => (
            <li
              key={goal.id}
              className="rounded-xl border border-slate-200/90 dark:border-surface-border bg-surface-elevated/80 dark:bg-surface-elevated/80 px-4 py-3"
            >
