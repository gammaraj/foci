-- Atomic guest → account migration + Realtime for shared task sync.

-- ── Realtime: collaborators receive task changes while viewing a share ──
do $$
begin
  if not exists (
    select 1
...
-- Needed so DELETE/UPDATE payloads include non-PK columns (project_id) for client filters.
alter table public.tasks replica identity full;

-- ── Atomic guest workspace migration (SECURITY DEFINER; forces auth.uid()) ──
...
revoke all on function public.migrate_guest_workspace(jsonb) from public;
